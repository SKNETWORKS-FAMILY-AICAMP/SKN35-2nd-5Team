...
def metric_table(frame: pd.DataFrame) -> Styler:
    shown = frame.copy()
...
with st.container(border=True):
    ...
    st.dataframe(metric_table(ml), hide_index=True, width="stretch")
with st.container(border=True):
    ...
    st.plotly_chart(style_plotly_chart(fig, 300), width="stretch")
...
with st.container(border=True):
    ...
    st.dataframe(dl_shown.style.format({metric: "{:.1%}" for metric in METRIC_LABELS}).background_gradient(subset=METRIC_LABELS, cmap="PuBuGn"), hide_index=True, width="stretch")
with st.container(border=True):
